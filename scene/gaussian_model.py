...
class GaussianModel_2dgs(nn.Module):
    ...
    def load_ply(self, path):
        ...
        self._loc_feature = nn.Parameter(
            torch.tensor(loc_feature, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(
                True))
...
class GaussianModel(nn.Module):
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float, loc_feature_size: int, speedup: bool):
        ...
        features[:, :3, 0] = fused_color
        features[:, 3:, 1:] = 0.0
        _loc_feature = torch.randn(fused_point_cloud.shape[0], loc_feature_size,
                                   1).float().cuda()
        _loc_feature = F.normalize(_loc_feature, p=2, dim=1)
